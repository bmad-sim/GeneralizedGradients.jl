...
gradient formalism of Venturini and Dragt~\cite{venturini1999} to a curvilinear coordinate
system which can have a curvature. This paper describes how the work of Van der Schueren
...
\end{align}
Needed in this calculation is
Van der Schueren Eqs.~(8) through (10)
\begin{align}
...
For the $\beta$ part, the gauge where $A_x = 0$ is used to produce
\begin{align}
  A_x &= 0 \\
  A_y &= \sum_{i,j = 0,0}^{\infty,\infty} \frac{1}{i+1} \, \beta_{s,i,j} \, x^{i+1} y^j \\
  A_s &= \frac{1}{1 + h \, x} \sum_{i,j = 0,0}^{\infty,\infty} \, \beta_{y,i,j} \, \left(
    \frac{x^{i+1}}{i+1} + \frac{h \, x^{i+2}}{i+2} \right) \, y^j
\end{align}
