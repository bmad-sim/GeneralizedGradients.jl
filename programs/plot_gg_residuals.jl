...
The data comes from `gg_make_fit_residual_table`, which is part of the package. Only the
...
Then `plot_residual_surfaces(gg_fit, field, plane)` for the three components of
one plane, or `plot_residual_component(gg_fit, field, component)` to sweep a
...
"""
    plot_residual_surfaces(gg_fit, field, plane; dplane = 0, component = 1:3) -> Figure
...
"""
function plot_residual_surfaces(gg_fit, field, plane::Integer;
                                dplane::Integer = 0, component = 1:3)
  r  = gg_make_fit_residual_table(gg_fit, field, plane; dplane)
...
"""
    plot_residual_component(gg_fit, field, component; planes = ...) -> Figure
...
"""
function plot_residual_component(gg_fit, field, component::Integer;
                                 planes = eachindex(gg_fit.z_base), ncol::Integer = 4)
  maps = [gg_make_fit_residual_table(gg_fit, field, p) for p in planes]
...
if abspath(PROGRAM_FILE) == @__FILE__
  ...
  gg_show_fit_residuals(gg_fit, field)

  worst = argmax(gg_fit.rms_weighted_plane)
  println("Worst plane is $worst (z = $(gg_fit.z_base[worst]) m).")

  fig = plot_residual_surfaces(gg_fit, field, worst)
  save(joinpath(@__DIR__, "gg_residual_plane_$worst.png"), fig)
  fig2 = plot_residual_component(gg_fit, field, 1)
