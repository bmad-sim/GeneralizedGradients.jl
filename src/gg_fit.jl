...
# (component, function) — one entry of the CB grids built in _run_fit.
...
function _run_fit(field::FieldGridTable, origin, n_planes_add, core_weight, outer_plane_weight,
...
# Fetch a parameter from the (sandbox) parameter module, or `default` if absent.
_param(p::Module, name::Symbol, default) = isdefined(p, name) ? getfield(p, name) : default
function _param(p::Module, name::Symbol)
  isdefined(p, name) || error("Parameter file must define `$name`.")
  return getfield(p, name)
end

"""
    gg_fit(param_file) -> output_file_path

Fit a 3D magnetic field grid to generalized-gradient (GG) coefficients
a_n(z), b_n(z), b_s(z) and their z-derivatives, plane by plane, writing the
result to an HDF5 file (readable by [`gg_load_fit`]).
...
gg_fit("fit_params.jl")
```
From the shell (see `programs/run_gg_fit.jl`):
```
julia programs/run_gg_fit.jl <param_file>
```

`param_file` is a Julia file that, when included, defines the fit inputs
(see "Input parameter file" below).  The output is written to `output_file`
(resolved relative to the parameter file's directory) and its path is returned.
...
The parameter file is a Julia file (e.g. "example/fit_params.jl") that defines:

  field               A `FieldGridTable` (typically via `read_field_grid`).
  origin = [x0, y0]   (x, y) line about which the GG coefficients are computed.
                      If field.g_ref is non-zero, origin must be [0, 0].
                      Default [0.0, 0.0].
  ...
  output_file         Name of the output HDF5 file. Default "gg_fit_result.h5".
...
### Example parameter file
  ...
  n_planes_add = 1
  core_weight = 1
  outer_plane_weight = 1
  output_file = "gg_fit_result.h5"

field.magnetic[ix,iy,iz] is the [Bx,By,Bz] 3-vector at the grid point, whose
(x, y, z) position is field.r0 + field.dr .* (ix, iy, iz).
"""
function gg_fit(param_file::AbstractString)
  param_path = abspath(param_file)

  # Load the fit inputs from the parameter file (sandboxed so its globals do
  # not leak into the package).  @__DIR__ inside the file resolves to its own
  # directory, so relative field-file paths work.
  p = Module(:GGFitParams)
  Base.include(p, param_path)

  field              = _param(p, :field)
  n_planes_add       = _param(p, :n_planes_add)
  origin             = _param(p, :origin, [0.0, 0.0])
  core_weight        = _param(p, :core_weight, 1)
  outer_plane_weight = _param(p, :outer_plane_weight, 1)
  output_file        = _param(p, :output_file, "gg_fit_result.h5")

  field isa FieldGridTable ||
    error("Parameter `field` must be a FieldGridTable, got $(typeof(field)).")
  ...
  result = _run_fit(field, origin, n_planes_add, core_weight, outer_plane_weight,
                    a_dicts, b_dicts, bs_dicts)

  # ---- Report ----------------------------------------------------------
  println("="^72)
  println("GG fit complete")
  println("  input file        : ", param_path)
  ...
  # interpret the fit later.  The (large) input field table is NOT stored.
  outfile = joinpath(dirname(param_path), output_file)
  gg_save_fit(outfile;
