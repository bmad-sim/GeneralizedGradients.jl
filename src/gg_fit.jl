...
Which unknowns exist at all is set by two cutoffs: `n_max` on the multipole index
and `m_max` on the derivative order. Giving either as a vector or range makes
`gg_fit` fit the full grid of combinations and keep the best one. For example

```julia
p.n_max = 1:10        # try n_max = 1, 2, … 10
p.m_max = 0:6         # crossed with m_max = 0, 1, … 6
```

runs 77 fits. A single `n_max`/`m_max` applies to every base plane — the model
...
- `n_max` — highest multipole index `n` kept. `-1` (the default) keeps every `n`
  in the coefficient table. A vector or range scans (see "Scanning" below).
- `m_max` — highest derivative order kept. `-1` (the default) means
...
function gg_fit(field::FieldGridTable, params::GGFitInputParams)
  ...
  # ---- Resolve the (n_max, m_max) models to try -------------------------
  # A scalar pins the model; a vector asks for a scan. Candidates are clamped to
  # what the coefficient table actually holds so that, say, `m_max = 0:100` does
  # not fit the same top model over and over.
  n_table_max = maximum(k[1] for d in (a_dicts..., b_dicts...) for k in keys(d))
  m_table_max = maximum(k[2] for d in (a_dicts..., b_dicts...) for k in keys(d))
  n_cands = _fit_candidates(params.n_max, n_table_max, n_table_max)
  m_cands = _fit_candidates(params.m_max, 2 * npa, m_table_max)
  scanning = !(params.n_max isa Int && params.m_max isa Int)
  ...
  for d in a_dicts, k in keys(d)
    k[1] <= n_max_all && k[2] <= m_max_all && push!(pset, (:a, k[1], k[2]))
  end
  for d in b_dicts, k in keys(d)
    k[1] <= n_max_all && k[2] <= m_max_all && push!(pset, (:b, k[1], k[2]))
  end
  for d in bs_dicts, m in keys(d)
    m <= m_max_all && push!(pset, (:bs, 0, m))
  end
  ...
  # unknowns describe a_0 and carry no multipole index, so `n_max` never drops them.
  cand_models = [(nx, mx) for nx in n_cands for mx in m_cands]
  cand_cols   = [[c for (c, (typ, n, m)) in enumerate(master_list)
                    if m <= mx && (typ == :bs || n <= nx)] for (nx, mx) in cand_models]
  ...
  for ((comp, typ), d) in comp_dicts
    for (key, terms) in d
      ...
      (m <= m_max_all && n <= n_max_all) || continue
  ...
  col_terms = [[Tuple{Int,Matrix{Float64}}[(mm, CB[(comp, typ, n, mm)])
                                           for mm in 0:j if haskey(CB, (comp, typ, n, mm))]
                for (typ, n, j) in master_list] for comp in 1:3]
  ...
  for (pidx, iz0) in enumerate(izs_grid)
    ...
    for (zi, iz) in enumerate(izs)
      ...
      # Taylor-extrapolation factors dz^k/k! for this plane offset, k = 0 … m_max_all.
      dzfac = [dz^k / factorial(k) for k in 0:m_max_all]
      for (iix, ix) in enumerate(ixs), (iiy, iy) in enumerate(iys)
        ...
        for comp in 1:3
          ...
          for col in 1:ncols_all
            j   = master_list[col][3]
            val = 0.0
            for (mm, grid) in terms_c[col]
              val += grid[iix, iiy] * dzfac[j-mm+1]
            end
            A[row, col] = val
          end
        end
  ...
  for c in 1:ncand
    ...
    push!(scan, GGFitScanPoint(; n_max = nx, m_max = mx, n_coef = ncoef,
  ...
  for (col, (typ, n, m)) in enumerate(params_list)
    typ == :a  && (a[(n, m)] = thetas[best][col, :])
    typ == :b  && (b[(n, m)] = thetas[best][col, :])
    typ == :bs && (bs[m]     = thetas[best][col, :])
  end

  return GGCoefs(; z_base, params = params_list, a, b, bs,
                    rms_plane = rms_c[best], rms_unweighted_plane = rmsu_c[best],
                    field_ave_plane, m_max, n_max, scan = scanning ? scan : GGFitScanPoint[],
...
Resolve an `n_max`/`m_max` input-parameter setting into the list of values
...
function _fit_candidates(spec::Union{Int,AbstractVector{Int}}, default::Int, table_max::Int)
  ...
  isempty(vals) && error("Empty n_max/m_max candidate list.")
  any(v -> v < 0, vals) && error("Negative n_max/m_max candidate in $spec.")
...
`(n_max, m_max)` scan table if a scan was run, a per-plane table of the weighted
...
function gg_fit_show_results(results::GGCoefs, field::FieldGridTable, params::GGFitInputParams)
  ...
  println("  n_max, m_max      : ", results.n_max, ", ", results.m_max,
  ...
  # ---- (n_max, m_max) scan table, when a scan was run --------------------
...
## HDF5 schema
    ...
    root attributes : g_ref, dz_grid (Float64); m_max, n_max, n_planes_add (Int);
...
function write_gg_fit(results::GGCoefs, field::FieldGridTable, params::GGFitInputParams)
  ...
  h5open(outfile, "w") do f
    ...
    attributes(f)["n_max"]              = Int(results.n_max)
